...
class GetDbUserPg(BaseMiddleware):
    async def on_process_message(self, message: types.Message, data: dict):  # data - данные, кот летят в хендлер
        ...
        if one_us is None:
            one_us = await db.add_user(int(message.chat.id), message.chat.full_name)

        data['user'] = \
             {'id_user': message.chat.id,
              'name': message.chat.full_name,
              'status': one_us["status"],
              'status_ru': one_us["status"],
              "new_user": one_us["new_user"],
              "channel": one_us["channel"],
              "habr_account": one_us["habr_account"]
              }
        # else:
        #     data['user'] = \
